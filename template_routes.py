...
def register_template_routes(app, login_required, api_key_or_login_required):
    ...
    def api_update_template(template_name):
        ...
        try:
            ...
            # 如果更新了模板代码，使用AI自动提取字段信息
            extracted_fields = None
            extract_error = None
            if content_updated:
                extracted_fields, extract_error = extract_template_fields_with_ai(data['content'])
            ...
            # 准备更新数据库的参数
            # 如果AI成功提取了字段，优先使用提取的字段；否则使用用户提供的字段（如果有）
            update_params = {
                'path': data.get('path')
            }

            if extracted_fields:
                update_params['server_address'] = extracted_fields.get('server_address', '')
                update_params['protocol_type'] = extracted_fields.get('protocol_type', '')
                update_params['port'] = extracted_fields.get('port')
                update_params['type'] = extracted_fields.get('type', 'default')
                update_params['api_address'] = extracted_fields.get('api_address', '')
                update_params['login_address'] = extracted_fields.get('login_address', '')
                update_params['redirect_address'] = extracted_fields.get('redirect_address', '')
            else:
                # 如果没有提取到字段，使用用户提供的字段（仅当用户明确提供时）
                if 'server_address' in data:
                    update_params['server_address'] = data.get('server_address')
                if 'protocol_type' in data:
                    update_params['protocol_type'] = data.get('protocol_type')
                if 'port' in data:
                    update_params['port'] = data.get('port')
                if 'type' in data:
                    update_params['type'] = data.get('type')
                if 'api_address' in data:
                    update_params['api_address'] = data.get('api_address')
                if 'login_address' in data:
                    update_params['login_address'] = data.get('login_address')
                if 'redirect_address' in data:
                    update_params['redirect_address'] = data.get('redirect_address')

            if web_dom is not None:
                update_params['web_dom'] = web_dom

            # 更新数据库记录
            update_template(template_name, **update_params)
            ...
            # 如果从代码中提取了字段，在响应中添加提取的字段信息
            if extracted_fields:
                response_data['extracted_fields'] = extracted_fields

            # 如果AI提取失败，在响应中添加警告信息
            if content_updated and extract_error:
                response_data['warning'] = f'字段自动提取失败: {extract_error}'
    ...
    def api_create_template():
        ...
        try:
            ...
            # 使用AI自动提取字段信息
            extracted_fields, extract_error = extract_template_fields_with_ai(content)

            # 如果提取成功，使用提取的字段；否则使用默认值
            if extracted_fields:
